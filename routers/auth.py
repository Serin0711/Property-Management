...
from database import Users, PropertyDetail
from oauth2 import AuthJWT
from schemas.userSchemas import UserSchema, UserSigninSchema, ForgotPasswordSchema, \
    ResetPasswordSchema, UpdatePasswordSchema, SocialSignupSchema
...
async def social_signup(
    ...
    db_user = Users.find_one({"email": social_data.email, "provider": "normal"})
    if db_user:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="User already exists with this email id as normal login, try normal login with email id and "
                   "password."
        )
...
@router.post("/signup")
async def signup_user(payload: UserSchema):
...
async def users_signin(payload: UserSigninSchema, response: Response, Authorize: AuthJWT = Depends()):
    try:
        ...
        db_user = Users.find_one({"email": payload.email})
        if not db_user:
            db_user = PropertyDetail.find_one({"email": payload.email})
            if not db_user:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Incorrect Email",
